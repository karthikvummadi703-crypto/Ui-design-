...
const DEFAULT_EFFECT_OPTIONS: EffectOptions = {
  ...
  fov: 90,
  fovSpeedUp: 140,
  speedUp: 6.5, // High hyper-drive acceleration boost!
  carLightsFade: 0.4,
  ...
  // Deep velvet-blue palette inspired by the spotlight studio backdrop
  colors: {
    roadColor: 0x010e2e,      // Dark royal blue road
    islandColor: 0x01081f,    // Even darker navy island
    background: 0x010617,     // Pitch velvet blue background
    shoulderLines: 0x1e3a8a,  // High contrast vibrant deep blue lines
    brokenLines: 0x3b82f6,   // Glowing bright blue center dashes
    leftCars: [0x2563eb, 0x1d4ed8, 0x60a5fa],  // Vivid neon deep blue streams
    rightCars: [0x3b82f6, 0x00f2ff, 0x1e40af], // Cyber gradient cyan and blue streams
    sticks: 0x00d2ff         // Radiant aqua sticks matching outer particle field
  }
};
...
const Hyperspeed: React.FC<HyperspeedProps> = ({ 
  ...
  // Adjust speeds dynamically based on React props changes
  useEffect(() => {
    if (appRef.current) {
      const app = appRef.current;
      if (isSpeedingUp) {
        app.fovTarget = options.fovSpeedUp;
        app.speedUpTarget = options.speedUp;
      } else {
        app.fovTarget = options.fov;
        app.speedUpTarget = 0;
      }
    }
  }, [isSpeedingUp, options.fov, options.fovSpeedUp, options.speedUp]);
  ...
  useEffect(() => {
    ...
    class App {
      ...
      constructor(container: HTMLDivElement, options: any = {}) {
        ...
        // Requirement 1: Move custom road flow extremely slowly by default (e.g., 0.06x baseline frequency)
        this.baselineSpeed = 0.06;
        ...
        const fogColorRaw = options.colors.background;
        const fog = new THREE.Fog(
          new THREE.Color(fogColorRaw),
          options.length * 0.15,
          options.length * 500
        );
        this.scene.fog = fog;
      ...
      update(delta: number) {
        // High quality speed-up/slow-down transitions
        const lerpPercentage = Math.exp(-(-60 * Math.log2(1 - 0.1)) * delta);
        this.speedUp += lerp(this.speedUp, this.speedUpTarget, lerpPercentage, 0.00001);

        // Advance custom clock. When speedUp holds (like during an option transition),
        // we speed up exponentially and smoothly.
        const activeSpeedFactor = this.baselineSpeed + this.speedUp;
        this.timeAccumulated += delta * activeSpeedFactor;

        const time = this.timeAccumulated;
    ...
    const mergedOptions = {
      ...options,
      distortion: distortions[options.distortion || 'turbulentDistortion'] || distortions.turbulentDistortion
    };

    const myApp = new App(container, mergedOptions);
    appRef.current = myApp;

    // Trigger initial speed state explicitly on boot
    if (isSpeedingUp) {
      myApp.fovTarget = options.fovSpeedUp;
      myApp.speedUpTarget = options.speedUp;
    } else {
      myApp.fovTarget = options.fov;
      myApp.speedUpTarget = 0;
    }
